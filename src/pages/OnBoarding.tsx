...
const OnBoarding = () => {
    ...
    useEffect(() => {
        if (selectedItem && selectedItem.id) {
            setApiForSelectedItem(
                `http://185.185.126.15:8000/api/management/onboarding/task_item?taskId=${selectedItem.id}`
            );
        }
    }, [selectedItem]);
    ...
    if (selectedLink && selectedLink.text === "Completed") {
        apiSelected =
            "http://185.185.126.15:8000/api/management/onboarding/task?status=completed";
    } else if (selectedLink && selectedLink.text === "In Progress") {
        apiSelected =
            "http://185.185.126.15:8000/api/management/onboarding/task?status=in_progress";
    } else if (selectedLink && selectedLink.text === "New") {
        apiSelected =
            "http://185.185.126.15:8000/api/management/onboarding/task?status=new";
    }
