...
import { getProductCatalog, sortProductCatalog, apiUrl } from '../../service/productcatalog';
...
export default function ProductCatalog() {
  ...
  const moveProfile = (id: any) => {
    ...
    setSelect(null);
    if (id) {
      history.push(`/product-profile/${id}`);
    }
  };

  const intial = (page: number, rowsPerPage: number) => {
    getProductCatalog(page, rowsPerPage)
        .then((resp) => {
          setLoading(false);
  ...
  const handleRequestSort = (
    ...
    sortProductCatalog(page, rowsPerPage, property, x).then((resp) => {
      setLoading(false);
      setRows(resp.data.content);
    });
  };
