...
import './AllBusinessCustomers.css'
...
import { getBusinessCustomers, sortBusinessCustomers, apiUrl } from '../../service/businesscustomers';
...
const EnhancedTableToolbar = ({
  ...
  return (
      ...
      >
        ...
        <Tooltip title='Create New Business Customer'>
          <Link to='new-business-customer'>
...
export default function AllBusinessCustomers() {
  ...
  const moveProfile = (id: any) => {
    ...
    setSelect(null);
    if (id) {
      history.push(`/business-customer-profile/${id}`);
    }
  };

  const intial = (page: number, rowsPerPage: number) => {
    getBusinessCustomers(page, rowsPerPage)
        .then((resp) => {
          setLoading(false);
  ...
  const handleRequestSort = (
    ...
    sortBusinessCustomers(page, rowsPerPage, property, x).then((resp) => {
      setLoading(false);
      setRows(resp.data.content);
    });
  };
